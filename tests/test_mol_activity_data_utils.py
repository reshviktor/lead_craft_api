...
from src.mol_activity.utils.mol_activity_data_utils import (
    ...
    convert_standard_units_to_pchembl,
    retrieve_pchembl_value,
    create_base_dataframe,
    retrieve_assay_info,
    determine_assay_type_auxiliary,
    retrieve_activity_status,
    attach_smiles,
    generate_complete_activity_dataframe,
    add_pchembl_values,
    save_activities_in_dataframe,
    create_certain_activity_mapper,
    generate_exact_assay_type,
    generate_approx_assay_type_for_row,
...
class TestChEMBLAPICallsToFetchTargetsAndActivities:
    ...
    @patch("chembl_webresource_client.new_client.new_client")
    def test_combine_activities_for_targets(
        self, mock_client, sample_activities, sample_activities_ex_2
    ):
        """Test combining activities from multiple targets"""
        stats = ConversionStatistics()
        mock_activity = Mock()
        mock_only = Mock()
        mock_only.only.side_effect = [sample_activities, sample_activities_ex_2]
        mock_activity.filter.return_value = mock_only
        mock_client.activity = mock_activity
        result = combine_activities_for_targets(
            ["CHEMBL2000", "CHEMBL2001"], stats=stats
        )
...
class TestUnitConversion:
    ...
    @pytest.mark.parametrize(
        "units_to_test, expected", [("uM", 6.0), ("µM", 6.0), ("um", 6.0), ("UM", 6.0)]
    )
...
class TestAttachSMILES:
    ...
    def test_attach_smiles_with_batching(self, mock_client, sample_molecule_structures):
        ...
        first_call_ids = mock_molecule.filter.call_args_list[0][1][
            "molecule_chembl_id__in"
        ]
        second_call_ids = mock_molecule.filter.call_args_list[1][1][
            "molecule_chembl_id__in"
        ]
    ...
    @patch("chembl_webresource_client.new_client.new_client")
    def test_attach_smiles_with_missing_structures(
        self, mock_client, sample_molecule_structures
    ):
        """Test handling of molecules without structure data."""
        df = pd.DataFrame({"molecule_chembl_id": ["CHEMBL7000", "CHEMBL7001"]})

        mock_molecule = Mock()
        mock_only = Mock()
        mock_only.only.return_value = [
            sample_molecule_structures[0],
            {"molecule_chembl_id": "CHEMBL7001", "molecule_structures": None},
        ]
        mock_molecule.filter.return_value = mock_only
        mock_client.molecule = mock_molecule
...
class TestAddPchEMBL:
    def test_add_pchembl_values_with_stats_tracking(
        self, sample_activities, sample_activities_ex_2
    ):
        ...
        assert result.loc[3, "pchembl_value"] is None or pd.isna(
            result.loc[3, "pchembl_value"]
        )
        assert pd.api.types.is_numeric_dtype(result["pchembl_value"])
        assert isinstance(result.loc[0, "pchembl_value"], (float, int)) or pd.isna(
            result.loc[0, "pchembl_value"]
        )
    ...
    def test_add_pchembl_values_empty_activities(self):
        ...
        df = pd.DataFrame({"activity_id": [], "molecule_chembl_id": []})
        activities = list()
        with pytest.raises(
            ValueError, match="DataFrame with activities should not be empty"
        ):
    ...
    def test_add_pchembl_values_mismatched_lengths(self, sample_activities):
        ...
        with pytest.raises(
            ValueError,
            match=(
                r"Number of activities provided \(3\) does not match "
...
class TestSaveActivitiesInDataframe:
    ...
    def test_save_activities_with_stats_tracking(
        self, mock_attach_smiles, sample_activities, sample_activities_ex_2
    ):
...
class TestRetrieveAssayInfo:
    ...
    def test_retrieve_assay_info(self, mock_client, sample_assay_info):
        ...
        mock_only.only.assert_called_once_with(
    ...
    @patch("chembl_webresource_client.new_client.new_client")
    def test_retrieve_assay_info_returns_correct_structure(
        self, mock_client, sample_assay_info
    ):
        """Test that returned data has correct structure"""
        df = pd.DataFrame(
            {"assay_chembl_id": ["CHEMBL001", "CHEMBL002"], "activity_id": [1, 2]}
        )
        mock_assay = Mock()
        mock_only = Mock()
        ...
        for assay_info in result:
            ...
            assert (
                "assay_cell_type" in assay_info
                or assay_info.get("assay_cell_type") is None
            )
            assert "bao_label" in assay_info

    def test_retrieve_assay_info_empty_dataframe(self):
        """Test handling of empty DataFrame"""
        df = pd.DataFrame({"assay_chembl_id": [], "activity_id": []})

        with pytest.raises(
            ValueError,
            match="Cannot retrieve assay information from empty activities dataframe",
        ):
            retrieve_assay_info(df)


@pytest.mark.parametrize(
    "bao_label,cell_type,expected",
    [
...
class TestGenerateExactAssayType:
    ...
    @patch(
        "src.mol_activity.utils.mol_activity_data_utils.create_certain_activity_mapper"
    )
    @patch("src.mol_activity.utils.mol_activity_data_utils.retrieve_assay_info")
    def test_generate_exact_assay_type_basic(
        self, mock_retrieve, mock_mapper, sample_assay_info
    ):
        ...
        mock_mapper.return_value = {"CHEMBL001": "biochemical", "CHEMBL002": "cellular"}
    ...
    def test_generate_exact_assay_type_empty_dataframe(self):
        """Test that empty DataFrame raises ValueError"""
        df = pd.DataFrame({"activity_id": [], "assay_chembl_id": []})
        with pytest.raises(
            ValueError,
            match="Cannot retrieve assay information from empty activities dataframe",
        ):
    ...
    @patch("src.mol_activity.utils.mol_activity_data_utils.retrieve_assay_info")
    def test_generate_exact_assay_type_partial_mapping(
        self, mock_retrieve, mock_mapper
    ):
...
class TestGenerateApproxAssayType:
    ...
    def test_generate_approx_assay_type_empty_dataframe(self):
        """Test that empty DataFrame raises ValueError"""
        df = pd.DataFrame(
            {"activity_id": [], "standard_type": [], "assay_type": [], "context": []}
        )
        with pytest.raises(
            ValueError,
            match="Cannot retrieve assay information from empty activities dataframe",
...
class TestActivityClassification:
    ...
    def test_activity_status_biochemical(self):
        ...
        assert result.loc[0, "is_active"]
        assert not result.loc[1, "is_active"]
        assert result.loc[2, "is_active"]
    ...
    def test_activity_status_relation_filter(self):
        ...
        assert result.loc[0, "is_active"]  # Valid relation in active region
        assert result.loc[1, "is_active"] is None  # Invalid relation in active region
        assert result.loc[2, "is_active"]  # Valid relation in active region
    ...
    def test_activity_status_invalid_relation_in_inactive_region(self):
        ...
        assert not result.loc[0, "is_active"]
        assert not result.loc[1, "is_active"]
        assert not result.loc[2, "is_active"]
    ...
    def test_activity_status_edge_cases_mixed(self):
        ...
        assert not result.loc[4, "is_active"]

    def test_activity_status_empty_dataframe_raises_error(self):
        """Test that empty DataFrame raises ValueError"""
        df = pd.DataFrame({"pchembl_value": [], "context": [], "relation": []})

        with pytest.raises(
            ValueError, match="Cannot classify activity status on empty DataFrame"
        ):
...
class TestRemoveDuplicateActivities:
    def test_empty_dataframe_raises_error(self):
        ...
        df = pd.DataFrame()
        with pytest.raises(
            ValueError, match="Activities dataframe should not be empty"
        ):
    ...
    def test_missing_columns_raises_error(self):
        ...
        df = pd.DataFrame(
            {"molecule_chembl_id": ["CHEMBL1"], "target_chembl_id": ["TARGET1"]}
        )
        with pytest.raises(
            ValueError,
            match="Column context not in dataframe columns during deduplication process",
        ):
            remove_duplicate_activities(df)
...
class TestIntegration:
    ...
    def test_complete_pipeline_no_targets_found(self, mock_client):
        ...
        with pytest.raises(
            ValueError, match="No targets found for query: 'NONEXISTENT'"
        ):
