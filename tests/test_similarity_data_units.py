...
from src.mol_activity.utils.similarity_data_utils import (
    ...
    get_tanimoto_similarity_for_query,
...
class TestCalculateTanimotoSimilarity:
    ...
    def test_identical_molecules(self):
        ...
        smiles = (
            "CC(N(C([C@H]1CC[C@@H](CC1)C)=O)c2c(C([O-])=O)cc(Oc3c(C(F)(F)F)cccn3)cc2)C"
        )
...
class TestGenerateSimilarityColumn:
    ...
    def test_custom_smiles_column(self):
        ...
        df = pd.DataFrame(
            {"my_smiles": ["CCO", "CCCO"], "molecule_chembl_id": ["CHEMBL1", "CHEMBL2"]}
        )
...
class TestSimilarityFilter:
    ...
    def test_empty_dataframe_raises_error(self):
        ...
        df = pd.DataFrame(
            {"tanimoto_similarity": [], "molecule_chembl_id": [], "pchembl_value": []}
        )
